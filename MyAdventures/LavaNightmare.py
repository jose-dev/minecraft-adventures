    ...
    while not gameOver:
        ...
        for hole in holes:
            mc.setBlocks(hole.x, hole.y, hole.z,
                         ...
                         block.AIR.id)
            mc.setBlocks(hole.x, hole.y - 1, hole.z,
                         hole.x, hole.y - 2, hole.z,
                         block.LAVA.id)
        time.sleep(2)
        #close up the holes
    ...
    while not gameOver and not levelComplete:
        ...
        for hit in hits:
            ...
            if blockHitType == block.DIAMOND_BLOCK.id:
                ...
                mc.postToChat("Diamonds remaining: {}".format(str(diamondsLeft)))
                if diamondsLeft == 0:
                    mc.postToChat("You only need to get to the end of the field to win!")
